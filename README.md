...
- [x] Phase 6 — CI, performance baseline, deployment, integration suite (`.github/workflows/ci.yml` three-job pipeline, `apps/api/Dockerfile` + `apps/web/Dockerfile` + `docker-compose.prod.yml`, `docs/perf-baseline.md`, `docs/runbooks/backup-recovery.md`, `docs/decisions/ADR-0006-threat-model.md`, `docs/privacy-review.md` with extended logger redactions; real-Postgres integration harness + 13 module specs covering every API surface; 11 Playwright e2e specs covering each UAT scenario; deterministic e2e fixture seeder `pnpm db:seed:e2e`)
...
scenarios. CI pipeline gates lint + typecheck + unit, integration
(ephemeral Postgres), and e2e + a11y on every PR.
